  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _ptail = 32.*M_PI*_rho*(1./(3*pow(_r_cut,6))-0.5)/(3.*pow(_r_cut,3));
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.dat");
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr.close();
        coutgr.open("../OUTPUT/gofr_blocks.dat");
        coutgr << "# BLOCK:        DISTANCE:        ACTUAL_GOFR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        coutpv.close();
        coutpv.open("../OUTPUT/pofv_blocks.dat");
        coutpv << "#    BLOCK:   VELOCITY:    ACTUAL_POFV:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pofv) {
    for (int i=0; i<_npart; i++) {
      ...
      _measurement(_index_pofv + static_cast<int>(velocity_temp/_bin_size_v)) += 1.0;
    }
    for (int i=0; i<_n_bins_v; i++) _measurement(_index_pofv + i) /= double(_npart);
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
...
void System :: averages(int blk){
  ...
  if (_measure_gofr) {
    for (int i = 0; i < _n_bins; i++) {
      double delta_V = 4.*M_PI*(pow((i+1)*_bin_size,3) - pow(i*_bin_size,3))/3.;
      _average(_index_gofr+i) /= _rho*_npart*delta_V;
    }
  }
  _global_av  += _average;

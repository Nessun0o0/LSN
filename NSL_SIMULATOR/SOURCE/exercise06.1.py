...
def edit_input(temp, H, restart, sim_type, nsteps, nblocks):
    """
    Edits the input.dat file of the NSL Simulator
    """
    input_data = None
    ...
    for i, line in enumerate(input_data):
        data = line.split(" ")
        if (data[0] == "TEMP"):
            input_data[i] = f"TEMP                   {temp:.1f}\n"
        elif (data[0] == "SIMULATION_TYPE"):
            data = [dato for dato in data if dato != ""] # Removes all the null elements from the data array
            ...
            input_data[i] = f"NBLOCKS                {nblocks}\n"
...
for i, simulation_type in enumerate(simulation_types):
    ...
    shutil.copy("../INPUT/CONFIG/config.ising", "../INPUT/CONFIG/config.xyz")

    if H_off:
        H = 0.0
        restart = 0
        simulation_steps = nsteps
        for temp in temps:
            if (temp <= 1.9):
            ...
            # Equilibration
            edit_input(temp, H, restart, simulation_type, int(simulation_steps), 1)
            ...
            # Simulation
            edit_input(temp, H, restart, simulation_type, int(simulation_steps), nblocks)
            os.system("./simulator.exe")

            for outfile in output_files:
                os.replace(f"../OUTPUT/{outfile}.dat", f"../OUTPUT-{sym_name}/{outfile}_{temp:.1f}.dat")
            
            simulation_steps *= 1.1

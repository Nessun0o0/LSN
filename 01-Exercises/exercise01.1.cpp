...
vector<double> Cumsum(const vector<double>& vec) {
    vector<double> cumsum_vec(vec.size());
...
// Part 1
    ...
    double N = 100;
    double L = M/N;
    vector<double> averages(N), averages2(N);
    vector<double> cumsum_averages(N), cumsum_averages2(N), errors_averages(N);
    ...
    for (int i = 0; i < N; i++) {
        ...
        errors_averages[i] = sqrt((cumsum_averages2[i] - pow(cumsum_averages[i], 2)) / static_cast<double>(i));
    }

    // Write results to file
    ofstream out;
    out.open("OUTPUT/averages.out");
    if (out.is_open()) {
        for (int i = 0; i < N; i++) {
            out << cumsum_averages[i] << " " << errors_averages[i] << endl;
        }
    } else cerr << "Error: unable to write averages.out" << endl;
    out.close();

   // Part 2

   vector<double> variances(N), variances2(N);
   vector<double> cumsum_variances(N), cumsum_variances2(N), errors_variances(N);
   ...
   if (out.is_open()) {
        for (int i = 0; i < N; i++) {
            out << cumsum_variances[i] << " " << errors_variances[i] << endl;
        }
   } else cerr << "Error: unable to write variances.out" << endl;
   out.close();
   ...
   // Write results to file
   out.open("OUTPUT/chi2.out");
   if (out.is_open()) {
        for (int i = 0; i < N; i++) {
            out << chi2[i] << endl;
        }
   } else cerr << "Error: unable to write chi2.out" << endl;
   out.close();
